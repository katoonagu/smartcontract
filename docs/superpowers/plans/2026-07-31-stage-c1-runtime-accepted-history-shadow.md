...
- [x] **Real-data follow-up: preserve the full predecessor planning boundary.** The first Task 10 replay proved that the exact 1,498-delta predecessor reconstructs 4,378 frontier states grouped into 888 unresolved history keys. A one-cycle/one-continuation assertion was therefore false: keeping the source target delta byte-identical also keeps its `d55…` predecessor head. The corrected runner permits at most two normal checkpoint cycles, never a third: planning-only, then exact target commit. The owning parser derives all continuation keys from predecessor frontier plus target delta, requires 889 planned rows, dense sequence, one capacity-one admission/reservation, consequent key last, equal disabled/enabled bytes, and zero provider calls. A PostgreSQL fixture with an inherited unresolved group is the runnable regression. The resulting transitive root exceeds the ordinary 2,000,000-node artifact budget, so only the C1 root serializer/parser uses a dedicated 8,000,000-node canonical budget; every nested artifact retains the ordinary limit and the CLI retains its 512 MiB file ceiling.
