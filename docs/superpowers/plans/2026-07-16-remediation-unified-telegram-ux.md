...
>
> **Staging amendment 2026-07-16:** после frozen RED batch разрешён отдельный
> boundary-test для Task 2 и изолированные staged typecheck gates для Tasks
> 2–8. Amendment фиксирует порядок исполнения, но не объявляет production-код
> реализованным.
...
- `tests/telegram/unifiedForensicRenderer.acceptance.test.ts` — core AC/REQ
  renderer tests.
- `tests/telegram/forensicPresentationContract.acceptance.test.ts` — narrow
  Task 2 boundary acceptance for canonical address references and invalid
  address fail-closed behavior; it imports no adapter, renderer or harness.
...
### 3.1 Staged typecheck gate for Tasks 2–8

Task 0's already-recorded full baseline remains historical evidence. After the
staging amendment, implementation Tasks 2–8 do **not** use the whole-repository
`npm run typecheck` as their per-task gate: the frozen Task 9 harness does not
exist yet and would make unrelated earlier tasks fail. Each of Tasks 2–8 runs
the same strict compiler options as the root `tsconfig.json`, but supplies only
that task's source files and focused tests as root files:

```powershell
$plan4TaskTypecheckFiles = @(
  # exact task-specific list from below
)

foreach ($file in $plan4TaskTypecheckFiles) {
  if (-not (Test-Path -LiteralPath $file)) {
    throw "plan4_staged_typecheck_file_missing:$file"
  }
}
...
npx tsc `
  ...
  --noEmit `
  $plan4TaskTypecheckFiles
if ($LASTEXITCODE -ne 0) { throw "plan4_staged_typecheck_failed" }
```

No task may add `@ts-ignore`, error filtering, `|| true`, reduced strictness or
different compiler options to pass this gate. Modules imported by these root
files are resolved and checked normally.

Exact root-file lists:

- **Task 2:** `src/telegram/forensicPresentation.ts`,
  `src/alerts/telegramHtml.ts`,
  `tests/telegram/forensicPresentationContract.acceptance.test.ts`.
- **Task 3:** `src/telegram/forensicResultRenderer.ts`,
...
- **Task 4:** `src/bot/wherePreliminaryNarrative.ts`,
  `src/bot/createBot.ts`,
  `tests/bot/unifiedTelegramModeWiring.acceptance.test.ts`,
  `tests/bot/wherePreliminaryNarrative.test.ts`, `tests/bot/createBot.test.ts`.
- **Task 5:** `src/bot/walletNarrativeSummary.ts`, `src/bot/createBot.ts`,
  `tests/bot/unifiedTelegramModeWiring.acceptance.test.ts`,
  `tests/bot/walletNarrativeSummary.test.ts`, `tests/bot/createBot.test.ts`.
- **Task 6:** `src/alerts/formatters.ts`,
...
- **Task 7:** `src/alerts/formatters.ts`, `src/approvals/approvalWorker.ts`,
  `src/bot/createBot.ts`,
  `tests/alerts/unifiedTelegramAlerts.acceptance.test.ts`,
  `tests/alerts/formatters.test.ts`,
  `tests/approvals/approvalWorker.test.ts`, `tests/bot/createBot.test.ts`.
- **Task 8:** `src/bot/walletNarrativeSummary.ts`,
...
Starting with Task 9, `npm run typecheck` is again a mandatory whole-repository
gate. Task 10 and final verification repeat the same full typecheck.
...
**Authorized staging correction:** после Task 1 отдельный docs-only amendment
добавляет описанный ниже Task 2 gate, а следующий отдельный test-only commit
создаёт только
`tests/telegram/forensicPresentationContract.acceptance.test.ts`. Сразу после
своего commit этот восьмой файл также становится frozen. Существующие семь
frozen-файлов, их тестовые названия и assertions не изменяются.
...
- `tests/telegram/forensicPresentationContract.acceptance.test.ts` — frozen,
  read-only run; it imports only the two Task 2 modules and no adapter,
  renderer or manual harness
...
The exact boundary tests are:

```text
[AC-09][PRESENTATION-CONTRACT] builds canonical references with exact first and last four characters
[REQ-38][PRESENTATION-CONTRACT] leaves invalid TRON addresses unlinked and safely escaped
```

The AC-09 case requires the exact first four and last four characters from the
full valid TRON address plus its canonical Tronscan URL. The REQ-38 case
requires an invalid address to render only as safely escaped plain text, never
as a link.

**RED before implementation:** run only the new boundary file; expect a module
resolution failure because `src/telegram/forensicPresentation.ts` and its
canonical reference helper do not exist yet.

```powershell
npx vitest run tests/telegram/forensicPresentationContract.acceptance.test.ts
```

**GREEN after implementation:** the same boundary command passes, then run the
exact Task 2 staged typecheck from §3.1. Renderer-level AC-39 and the remaining
renderer-level REQ-38 cases are deliberately not Task 2 gates; Task 3 owns
them.
...
npx vitest run tests/telegram/unifiedForensicRenderer.acceptance.test.ts `
  -t "AC-07|AC-08|AC-09|AC-12|AC-13|AC-39|REQ-06|REQ-07|REQ-08|REQ-09|REQ-11|REQ-31|REQ-32|REQ-33|REQ-34|REQ-38"
```

Expected: legacy output lacks unified subject, route, coverage and no-final
invariants.

**GREEN:** same command, then the exact Task 3 staged typecheck from §3.1.
This is the first renderer-level gate for AC-09, AC-39 and the renderer-level
remainder of REQ-38.
...
**GREEN:** same command plus focused legacy tests and the exact Task 4 staged
typecheck from §3.1.
...
**GREEN:** same command, affected bot tests and the exact Task 5 staged
typecheck from §3.1.
...
**GREEN:** same command plus affected Incoming job/formatter tests and the
exact Task 6 staged typecheck from §3.1.
...
**GREEN:** same command, affected Approval tests and the exact Task 7 staged
typecheck from §3.1.
...
**GREEN:** run all three files without `-t`, then the exact Task 8 staged
typecheck from §3.1.
...
Task 9 is the first post-staging task with mandatory full-repository
`npm run typecheck`. A staged subset is no longer sufficient from this point.
...
| REQ-38 UX | Task 2 boundary safely escapes and does not link an invalid TRON address; Task 3 owns renderer fail-closed behavior; Task 8 closes legacy negatives | 2, 3, 8 | `[REQ-38][PRESENTATION-CONTRACT] leaves invalid TRON addresses unlinked and safely escaped`; `[REQ-38] fails closed for invalid addresses facts and legacy denominators` |
...
| AC-09 | Task 2 boundary preserves exact first/final four characters and canonical URL; Task 3 proves renderer coverage | 1, 2, 3 | `[AC-09][PRESENTATION-CONTRACT] builds canonical references with exact first and last four characters`; `[AC-09] safely shortens and links every valid TRON address`; `[AC-09][ADDRESS-SUFFIX] preserves the exact last four TRON address characters without reordering or omission` |
...
| AC-39 Plan 4 regression | Unified renderer cannot reintroduce LLM text; renderer gate begins in Task 3, then wiring/compatibility repeat it | 1, 3, 5, 8 | `[AC-39][UNIFIED-RENDERER] excludes every legacy LLM field and heading` |
...
- [x] Authorized Task 2 boundary acceptance imports only
      `forensicPresentation.ts` and `telegramHtml.ts`; it does not import the
      adapter, renderer or Task 9 harness.
- [x] Existing seven frozen files and their test names stay unchanged; the new
      boundary file becomes the eighth frozen file in its own test-only commit.
- [x] Tasks 2–8 use strict isolated staged typecheck roots with unchanged root
      compiler options; Task 9, Task 10 and final verification require full
      `npm run typecheck`.
...
ограниченных commit перед Task 2: сначала этот docs-only amendment, затем
test-only commit только с
`tests/telegram/forensicPresentationContract.acceptance.test.ts`. Ни один
production-файл на этом staging-шаге не изменяется.
