...
FastCheck не заменяет эти режимы. Он может передать DeepCheck короткий пакет подсказок: какие важные соседние адреса стоит учитывать. Where is money при этом запускается отдельно и проверяет происхождение денег своей логикой. Дальше каждый режим сохраняет отдельный результат.
...
- уровень риска;
- список причин;
- быстрый снимок риска для итогового риска и подсказок DeepCheck;
...
- собственный контекст выбранной суммы, recent-flow или drain episode.
...
## Как Режимы Делятся Данными Сейчас

FastCheck может создать небольшой пакет подсказок для DeepCheck. Это помогает DeepCheck выбрать важные соседние адреса, но DeepCheck все равно запускает собственный анализ профиля и контекста.

Where is money не зависит от FastCheck как от proof engine. Он запускает source-of-funds логику вокруг выбранной суммы, recent-flow или drain episode.

То есть режимы связаны операционно, но их не нужно описывать как один большой pipeline, где FastCheck полностью кормит все остальное.
...
flowchart LR
  ...
  A --> W["Where is money"]
  A --> D["DeepCheck"]
  F -- "подсказки по важным соседям" --> D
...
Where is money запускается рядом с другими режимами, но проверяет происхождение денег своей логикой.
