...
attests the candidate or rollback worktree. Before a start it checks both
`git ls-files -v` and `-f`, rejects every non-normal tracked index flag including
fsmonitor-valid, then disables fsmonitor/untracked-cache use and pins normalized `src/index.ts` bytes to the exact
