...
3. Preserve the admitted real Stage C evidence and Task 2 proof: one frozen
   `200/200` role map exists and the prerequisite audit passes. Tasks 5-7 wire
...
4. Continue the approved C0-C6 implementation plan suite. Do not advance C0a
   or C2-C4 past the blocked physical-population Task 3. C1 Tasks 3-10 are
   ...
   capacity-one barrier. Task 10 admits the three real-history evidence files
   from tested source commit `d480b7839c0c61020e9f8c3b4d2e79af5a496e87`.
   Unavailable cashflow authority does not block that completed C1 slice.
...
| Stage C | Incomplete. C0 Tasks 1-2 and C1 Tasks 1-10 are implemented; Task 10 admits the three real-history artifacts generated from tested source commit `d480b7839c0c61020e9f8c3b4d2e79af5a496e87`. The retained legacy corpus gate passes `24/24 + 6/6`, and the isolated disabled/enabled replay preserves 889 planned rows, admits/reserves only the first, makes zero provider calls, and leaves authoritative projections equal. C0 Task 3 is stopped before implementation because its plan-only query/kind literals have no exact owning schemas/codecs/fixtures/producers. C0b has not started. Physical-page/EOA/order/balance authority, the new blind set, and cashflow shadow remain absent | C1 admission is complete. Keep C2-C6 deferred to separately gated passes and all current Stage C work score-neutral |
...
production scoring, traversal result and delivery remain unchanged. The C1
foundation keeps `UNIFIED_SERVICE_ROLE_SHADOW_POLICY` disabled by default;
Tasks 6-7 wire the exact enabled literal only to standalone score-neutral
observation, post-checkpoint/startup receipts, and completed-run summaries.
...
adds the optional bounded coordinator observer and complete-subgroup precommit
path. Task 6 wires it behind the exact disabled-by-default policy and writes a
...
bounded two-cycle runtime proof and self-contained C6 root parser. Task 10
admits the input, identity, and acceptance root generated from tested source
commit `d480b7839c0c61020e9f8c3b4d2e79af5a496e87`.
...
Execution checkpoint after C1 Task 10 admission:
...
- C1 Tasks 1-2 landed through `28ecd6f3`; strict config and frozen-V1/additive-V2
  binding tests pass `19/19`. Task 3 landed through `1028c2a7`; its acceptance
...
- C1 Task 4 is complete. One cached promise per active initialization freezes
  ...
  hashes without a V2 scan, and compound lookup reads only that validated set.
  Only an initial no-fence scan may classify corrupt wrapper keys or source
  closure as malformed while retaining only sorted valid hashes. A pre-existing
  ready fence with a non-reusable closure deterministically publishes or reuses
  conflict, and its row set stabilizes after the first such restart. The normal
  attempt plus at most two fresh same-lock
  ...
  retry and rescan. The unit file passes `24/24`; the real schema-037 PostgreSQL
  file passes `7/7` with zero skips, the combined Task 3+4 file set passes
  `25/25`, and typecheck passes.
- C1 Task 5 is implemented. The coordinator supplies owned deep copies after
  accepted-group delta persistence, bounds the optional observer at 1,000 ms,
  and keeps heartbeat errors authoritative. Frozen exact-map subgroups produce
  per-state profiles and one idempotent strict unconfirmed precommit;
  insufficient diagnostics are not qualifying. Abort seen before the final
  insert settles/transaction callback returns rolls back the subgroup. A COMMIT
  already ordered after all checks is complete and may become Task 7 recovery
  input if its local token is lost. The coordinator/runtime files pass `26/26`
  and `31/31`.
- C1 Task 6 is complete in code. The worker's 1,000 ms post-durable callback
  forwards exact normalized checkpoint authority, the ordered transaction
  returns only identities it validated and committed, and enabled runtime
  reconciliation requires one manifest match plus candidate-delta ancestry.
  Unrelated entries from the same atomic prefix are bound into the receipt;
  zero/duplicate matches, cancellation and unproved ancestry write none.
  Pending handoff retains only exact identity plus precommit hashes, is bounded
  to 512 attempt buckets for twice the worker lease, and is retired in `finally`
  for every post-durable outcome. Reconciliation sets 500 ms local lock and
  statement timeouts before authority reads. The relevant unit set passes
  `95/95`; shadow-runtime PostgreSQL passes `8/8` and ordered-commit PostgreSQL
  passes `18/18`, both with zero skips.
- C1 Task 7 is complete in code. Enabled startup performs one hard-bounded,
  non-polling recovery sweep over non-cancelled durable precommits and every
  non-cancelled completed traversal. It acquires only from a startup-owned,
  single-connection PostgreSQL pool with a native 400 ms connection timeout,
  then closes that pool in `finally`; authoritative work keeps its existing
  main pool unchanged. An internal 700 ms absolute budget plus 150 ms database
  deadlines leaves cleanup headroom below the 1,000 ms public ceiling. Receipt
  recovery still requires current authoritative commit closure;
...
- C1 Task 9 implements the exact source-input/identity/root contracts, strict
  ...
  real storage boundaries. Task 10 admits the input, identity, and root from
  tested source commit `d480b7839c0c61020e9f8c3b4d2e79af5a496e87`; the root
  SHA-256 is
