...
describe("adminConsoleHtml", () => {
  ...
  it("uses rendered evidence edges for selected service connected neighbors", () => {
    const html = adminConsoleHtml();
    const block = html.slice(html.indexOf("function edgeHasNeighborMoneyEvidence"), html.indexOf("function selectedNodeTransferEdges"));

    expect(block).toContain("state.renderedEdgesById.values()");
    expect(block).toContain("edgeHasNeighborMoneyEvidence(edge)");
    expect(block).toContain('type === "contract_call_context"');
  });

  it("renders only stored-money service connected neighbors from rendered edges", () => {
    const html = adminConsoleHtml();
    const block = html.slice(html.indexOf("function edgeHasNeighborMoneyEvidence"), html.indexOf("function selectedNodeTransferEdges"));
    const api = new Function(`
      const rawEdges = [{
        ...
        fromNodeId: "service",
        toNodeId: "wallet-a",
        metadata: { evidenceType: "grouped_transfers", aggregateTransferCount: 2, aggregateAmountFormatted: "42 USDT" }
      }];
      const state = { renderedEdgesById: new Map([
        ["rendered-money", {
          id: "rendered-money",
          fromNodeId: "wallet-b",
          toNodeId: "service",
          metadata: { evidenceType: "boundary_context", underlyingTransfers: [{ txHash: "tx-b" }] }
        }],
        ["context-only", {
          id: "context-only",
          fromNodeId: "wallet-c",
          toNodeId: "service",
          txHash: "tx-context",
          metadata: { evidenceType: "contract_call_context" }
        }],
        ["dup-edge", {
          id: "dup-edge",
          fromNodeId: "service",
      ...
      function filteredTransferEdges() { return rawEdges; }
      function edgeEvidenceType(edge) { return edge?.metadata?.evidenceType || (edge?.type === "transfer" ? "direct_transfer" : "unknown"); }
      function edgeIsPeerLink() { return false; }
      function edgeHasTransferRows(edge) { return Array.isArray(edge?.metadata?.underlyingTransfers) && edge.metadata.underlyingTransfers.length > 0; }
      function edgeHasStoredMoneyEvidence(edge) { return Boolean(edge?.metadata?.aggregateTransferCount && edge?.metadata?.aggregateAmountFormatted); }
      function edgeAggregateTransferCount(edge) { return edge?.metadata?.aggregateTransferCount || (Array.isArray(edge?.metadata?.underlyingTransfers) ? edge.metadata.underlyingTransfers.length : null); }
      function edgeAggregateAmountLabel(edge) { return edge?.metadata?.aggregateAmountFormatted || ""; }
      function edgeCanvasLabel() { return ""; }
      function nodeById(id) { return { id, address: id }; }
      function nodeAddress(node) { return node?.address || ""; }
      function edgeDetailedAmountLabel(edge) { return edgeAggregateAmountLabel(edge); }
      function edgeCanvasAmountLabel() { return ""; }
      function edgeTime() { return ""; }
      function txDetailLink(tx) { return "tx:" + tx; }
      function addressDetailLink(address) { return "addr:" + address; }
      function escapeHtml(value) { return String(value); }
      ${block}
      return { connectedNeighborLines };
    `)() as { connectedNeighborLines(node: { id: string }): string[] };

    const lines = api.connectedNeighborLines({ id: "service" });

    expect(lines).toHaveLength(2);
    expect(lines.join("\n")).toContain("addr:wallet-a");
    expect(lines.join("\n")).toContain("addr:wallet-b");
    expect(lines.join("\n")).not.toContain("addr:wallet-c");
    expect(lines.filter((line) => line.includes("addr:wallet-a"))).toHaveLength(1);
  });
});
