...
import { takeoverCleanupOnlyProductionOperationLeaseV2 } from "../src/release/productionOperationStore";
...
export async function runTakeoverCleanupOnlyProductionOperationLease(
  args: string[],
  now = () => new Date().toISOString()
) {
  if (args.length !== 2 || !/^[0-9a-f]{64}$/u.test(args[0]!)) {
    throw new Error("usage: release:production:lease:cleanup-only-takeover <old-lease-sha> <protected-artifact-root>");
  }
  ...
  const result = await takeoverCleanupOnlyProductionOperationLeaseV2({
    artifactRoot: root,
    expectedOldLeaseSha256,
    evaluatedAt: now()
  });
  console.log(JSON.stringify({ status: "passed", operationId: result.takeover.operationId,
    terminalReason: result.takeover.terminalReason, cleaned: true }));
  return result;
}

if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  runTakeoverCleanupOnlyProductionOperationLease(process.argv.slice(2)).catch((error) => {
